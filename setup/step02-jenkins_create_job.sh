...
wget -O $JK_CLI_PATH http://$JENK_HOST_EXT/jnlpJars/$JK_CLI_JAR
...
java -jar $JK_CLI_PATH -s http://$JENK_HOST_EXT/ -auth $JENK_USER:$JENK_PASS create-job deploy-webapp < $CUR_DIR/jenkins/jobs/config-deploy-webapp.xml
java -jar $JK_CLI_PATH -s http://$JENK_HOST_EXT/ -auth $JENK_USER:$JENK_PASS create-job deploy-webapp-with-grafana < $CUR_DIR/jenkins/jobs/config-deploy-webapp-with-grafana.xml
